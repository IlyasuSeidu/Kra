...
Station sign-in must use the shared staff auth component. It must not fork a station-specific auth form.

Staff activation must route to `/(auth)/staff/activate` when an invite, activation token, or inactive staff status requires activation before station access.

Support and supervisor contact must route to shared ops support with auth context when a session exists. External contact fallback is allowed only when no session context is available.

Redirect after sign-in must go through the role home gate first. The station overview opens only after the role gate confirms valid station scope.

Platform follow-up decision: add a dedicated staff session introspection response for mobile that returns role, station scope presence, user status, and safe display labels without requiring screens to inspect provider claim details directly.
