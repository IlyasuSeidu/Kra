...
- Exact station display label source.
- Exact cached summary retention window.
- Exact bucket thresholds for overdue visual treatment once SLA data is available.
- Exact recent work row count if product later chooses fewer than three.

Future backend/platform improvement:
- Add a station overview aggregate endpoint that returns station bucket counts, overdue markers, blocked reasons, and freshness in one role-scoped response so mobile does not rely on local bucket mapping over list rows.
