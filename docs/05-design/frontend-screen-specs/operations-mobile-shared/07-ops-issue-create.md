...
Category selection must use segmented category cards. Severity selection must use an accessible radio-list pattern, and P1 severity must require an explicit confirmation step before submission.

Issue drafts must retain for 24 hours and must clear immediately after a successful submit, explicit discard, or account sign-out.

Duplicate issue guidance must use only role-safe issue summaries. If a duplicate summary is available, show a compact warning with status, category, severity, and last update; do not expose restricted support notes.

Stale delivery context must block submission after 5 minutes for active custody actions and after 15 minutes for non-action contexts. The screen must require refresh before submit when context is stale.

If issue creation is blocked, the fallback route is `/(ops)/support` with delivery context attached when available.

Platform follow-up decision: add a typed `list issues by delivery` mobile contract with role-safe summaries so duplicate guidance can be precise without exposing restricted support detail.
