...
- Exact wording of rate-limit wait text.
- Exact synced/discarded retention window.
- Exact audit expansion layout.
- Exact secure-store adapter.
- Exact backend refresh timeout.

Future backend/platform improvement:
- Add explicit typed error codes for idempotency payload mismatch, same-delivery dependency blocked, payload unavailable, support-required discard, and proof dependency missing so frontend classification can avoid relying on `VALIDATION_ERROR` details.
