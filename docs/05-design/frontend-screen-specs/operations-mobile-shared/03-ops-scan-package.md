...
- Exact camera scanning library.
- Exact barcode formats supported, as long as package labels used by operations are supported.
- Exact offline queue mechanism, as long as custody is not shown as confirmed before backend success.
- Exact supervisor approval UI, as long as the request receives a verified `supervisorOverrideActorId`.
