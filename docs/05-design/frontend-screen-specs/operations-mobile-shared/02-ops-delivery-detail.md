...
- Exact native component library wrappers.
- Exact station display-name resolver.
- Exact freshness threshold, as long as stale state is visible and configurable.
- Exact route names for role workflows, as long as `/(ops)/deliveries/:deliveryId` remains the shared detail entry.
