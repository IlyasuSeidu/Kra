...
- Exact station display-name resolver.
- Exact disclosure pattern for long evidence rows.
- Exact admin audit route if configured.

Future backend improvement:
- Add `fallbackUsed` and `supervisorOverrideActorId` or safe derived fallback fields to `deliveryTimelineEntrySchema.metadata` for handoff events if mobile custody review must show fallback approval directly.
