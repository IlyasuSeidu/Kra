...
Do not use a full interactive map unless the implementation has approved station coordinates, accessible list fallbacks, and performance control. Do not use speculative coverage circles, invented live vehicles, unapproved city pins, generic Africa maps, or "nationwide soon" decoration.
...
- Invented heatmaps.
- Invented "coming soon" city pins.
...
- Use exact language.
- Avoid invented volume or performance metrics.
...
- Does not include invented metrics.
...
- Route checker and directory show skeletons or calm loading panels.
- Loading copy: `Loading current service areas...`
- Do not show invented stations while loading.
...
- Add invented upcoming city pins.
- Expose staff assignments or internal capacity.
- Use prototype route data outside approved v1 launch data.
...
- Render the route visual as an inline accessible SVG paired with a text-first station and corridor list. Do not use raster maps, canvas maps, or third-party map tiles for v1 static launch content.
- The route checker uses approved static v1 launch data as the source of truth for this page. If a public station endpoint later exists, it must be consumed through a typed adapter that preserves the same user-facing states and falls back to the approved static launch data only when policy allows stale public content.
- `/coverage` is a canonical alias that redirects permanently to `/service-areas` at the hosting/router layer when possible. If only client routing is available, it must replace history with `/service-areas` immediately and set canonical metadata to `/service-areas`.
