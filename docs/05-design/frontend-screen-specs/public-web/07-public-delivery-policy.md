...
Do not turn policy into dense legal copy or a decorative timeline. Avoid invented ETAs, broad guarantee claims, vague "we always try our best" language, and unsupported live-map promises.
...
- Invented live tracking maps.
- Invented ETA countdowns.
...
- Show:
  ...
  - `Pickup or doorstep`
  - `Proof`
- Do not use invented package data.
...
- No invented live tracking movement.
- No countdown timers.
...
- Does not render invented live ETA or GPS promise.
...
- Lifecycle stages are always visible as public-language stage cards. Each card includes status meaning, required gate, user action, and support trigger; secondary operational detail may be expanded within the same card but the core stage cannot be hidden.
- Pickup hold and return-to-sender timing render as milestone cards with a subtle connecting rail on desktop and stacked cards on mobile. Cards must show `72 hours`, `on_hold`, `7 calendar days`, and return-to-sender eligibility without implying automatic return.
- Desktop uses sticky in-page navigation after the hero for `Lifecycle`, `Doorstep attempts`, `Station pickup`, `Proof`, `Support`, and `FAQ`; mobile uses normal document flow anchors only.
