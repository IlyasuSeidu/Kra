...
Do not turn trust into vague badges. Avoid shield-icon spam, invented certifications, generic safety claims, invented audit logs, stock photos of warehouses, or intimidating legal language.
...
- Must not render invented tracking records or invented audit logs.
...
- Does not show invented delivery records.
...
- No invented scanning animation that looks like live data capture.
...
- Add invented tracking records.
- Add invented audit logs.
...
- The proof matrix is table-first on desktop and becomes accessible proof cards below `720px`; every mobile card must repeat actor, custody trigger, required proof, and failure path.
- The hero ledger visual is an inline accessible SVG with adjacent text labels and CSS state styling. Do not use raster ledger imagery, canvas-only drawing, or animation that looks like live operational data.
- Desktop uses sticky in-page navigation after the hero for `Proof chain`, `Handoffs`, `Receiver proof`, `Fallback controls`, `Privacy boundary`, and `FAQ`; mobile uses normal document flow anchors only.
