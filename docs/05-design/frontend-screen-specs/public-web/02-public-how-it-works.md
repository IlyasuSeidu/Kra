...
- If no production sender entry route exists, `Start a delivery` must route only to `/service-areas` or another production-approved route.
...
Do not create a decorative timeline that looks impressive but teaches nothing. Avoid generic courier icons, invented GPS maps, invented tracking records, motion-heavy animations, stock delivery photos, and empty "seamless" language.
...
- Must not invent live delivery examples or unverified package data.
...
- Use a `CustodyRoutePreview` visual, not an invented dashboard.
...
- The page states public tracking is event-first, not live GPS simulation.
...
- Avoid prototype data files that look like production state.
- Avoid invented delivery IDs, invented people, invented tracking records, or invented map pins.
...
- Add unsupported "coming soon" sections.
...
- Process details use progressive disclosure after the hero: each step shows a short visible summary, with one expanded detail panel at a time on mobile and all detail summaries scannable on desktop.
- The route diagram is an inline accessible SVG supported by adjacent text, not a raster asset or canvas drawing, so labels, contrast, and responsive behavior stay testable.
- Desktop uses a sticky chapter navigation rail after the hero; mobile uses inline section anchors only when they do not crowd the first viewport.
...
- Unverified GPS tracking promise.
