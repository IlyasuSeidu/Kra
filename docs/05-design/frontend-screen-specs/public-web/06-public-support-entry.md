...
Do not build a noisy help center homepage. Avoid chat bubbles that do nothing, invented live-agent availability, generic smiling-agent stock photos, endless FAQ grids, or refund-promising banners.
...
- Header does not add invented chat controls.
...
- Do not show real issue IDs or invented support conversations.
...
- Answer: `If a backend issue is created, show the confirmed issue reference. If the page only prepares a public contact request, do not show an invented issue reference.`
...
- Do not show invented issue reference.
...
- No invented "agent typing" animation.
...
- Do not rely on input hint text as labels.
...
- No dependency is added solely for invented live support.
...
- Submitted fallback state does not show invented issue reference.
...
- Public unauthenticated fallback prepares a structured contact summary on the page and opens the configured support email/contact channel only after explicit user action. The submitted state must say `Support request prepared`, not claim a backend issue exists.
- Authenticated users route to the role-appropriate app support or issue-creation route when that route exists. If no authenticated route exists for the current role, the page uses the same public contact-summary fallback.
- Reason guidance expands inline under the selected reason, with a compact desktop summary panel mirroring the selected reason, required context, and next action. Mobile uses inline guidance only.
...
Pass. The spec defines a calm, evidence-led support entry with quick paths, triage, adaptive guidance, submission states, policy expectations, urgency guidance, accessibility, privacy, and strict anti-invented-support rules.
...
- Public fallback prepares or routes support contact without invented issue creation.
