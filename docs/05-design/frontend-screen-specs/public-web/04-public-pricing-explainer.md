...
- If no production sender entry route exists, `Start a delivery` routes to `/service-areas` or an approved waitlist route, not a demo form.
...
## Restraint Rule
Do not build a fake pricing calculator. Do not add sliders, estimated totals, fake discounts, countdown offers, animated savings meters, or comparison charts against unnamed competitors.
...
- Avoids fake live checkout data.
...
- Add fake checkout data.
- Add discounts, coupons, or promotional pricing.
- Add competitor comparison.
- Add unapproved route fees.
- Add demo or sample pricing outside approved v1 rules.

## Open Decisions
No product decision blocks this static pricing explainer.

Implementation may choose:
- Whether surcharge groups are rendered as always-open cards or accessible tabs.
- Whether the hero receipt visual includes one static example.
- Whether route fee tables become cards below a specific breakpoint.
