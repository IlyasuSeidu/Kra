...
- Exact station display-name lookup source.
- Whether home shows three or five delivery cards on large phones.
- Whether create delivery is a fixed button or inline primary card.
- Whether notification preview appears above or below delivery list when there are no active delivery blockers.
- Final status chip color mapping.

These must not change backend authority, route ownership, or data safety.
