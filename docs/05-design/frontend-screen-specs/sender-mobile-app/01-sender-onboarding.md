...
Non-blocking design decisions for implementation:
- Final font pair.
- Final product mark asset.
- Exact color token values.
- Whether public tracking opens in-app or web view.
- Whether route rail uses icon nodes or text-only nodes on narrow phones.

These are implementation-time visual system choices and must not change product behavior.
