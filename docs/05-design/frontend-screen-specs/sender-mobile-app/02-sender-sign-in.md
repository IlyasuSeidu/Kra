...
After an OTP challenge starts, phone masking must preserve the country code and last 2 digits only.

OTP entry must use one accessible input with a visually grouped cell treatment. It must not use separate inaccessible fields that break paste, screen readers, or platform autofill.

Resend countdown must display only when the backend returns a lock or resend time. If no backend time is available, the screen must show retry eligibility through action state only.

Typography and color must use shared design-system tokens. Screen-local type or color values are not allowed.
