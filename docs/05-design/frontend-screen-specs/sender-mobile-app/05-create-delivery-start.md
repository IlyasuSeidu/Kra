...
Local delivery drafts expire after 24 hours.

The v1 local draft store key is `sender:create-delivery:draft:v1`.

Offline draft entry is allowed only for non-price, non-payment fields. Quote, price confirmation, payment, and final delivery creation must be blocked until online backend authority is available.

Bottom tabs must be hidden during the create-delivery flow to keep attention on booking completion and reduce accidental route loss.

The booking path rail must be text-first. Icons can be decorative support only and must not be the sole indicator of step meaning.
