...
The phone country selector defaults to Ghana. A country picker is shown only when multi-country sender access is enabled by configuration.

Support must route in-app to the sender support or recovery route when signed-in or recovery context exists. Without app context, the fallback destination is public `/support`.

Support must open in-app when the route exists. External web fallback is allowed only when no in-app support route is available.

Resend timing must show whenever the backend returns a lock or resend time. If no backend time is available, the screen must not invent a countdown.

The recovery icon must use a shared design-system icon treatment. It must not imply stronger identity verification or account protection than the backend actually provides.
